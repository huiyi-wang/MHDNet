...
class Visualizer:
    ...
    def save_rank_result(self, query_indices, output, max_rank=5, vis_label=False, label_sort='ascending',actmap=False):
        if vis_label:
        ...
        for cnt, q_idx in enumerate(tqdm.tqdm(query_indices)):
            ...
            query_img = query_info[0]

            cam_id = query_info[2]
            # query_name = query_info[4].split('/')[-1]
            all_imgs.append(query_img)
            query_img = np.rollaxis(np.asarray(query_img.numpy()), 0, 3)
            ...
            for i in range(max_rank):
                ...
                gallery_img = gallery_info[0]
                cam_id = gallery_info[2]
                all_imgs.append(gallery_img)
                # gallery_img = np.rollaxis(np.asarray(gallery_img, dtype=np.uint8), 0, 3)
                gallery_img = np.rollaxis(np.asarray(gallery_img), 0, 3)
                if cmc[i] == 1:
                    label = 'true'
    ...
    def vis_rank_list(self, output, vis_label, num_vis=100, rank_sort="ascending", label_sort="ascending", max_rank=5,actmap=False):
